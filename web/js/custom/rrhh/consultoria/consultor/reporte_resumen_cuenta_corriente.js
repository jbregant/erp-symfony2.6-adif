...
dt_table_reporte = dt_datatable($('#table-reporte'), {
    ajax: {
        url: __AJAX_PATH__ + 'consultor/index_table_reporte_resumen_cuenta_corriente/',
        data: function (d) {
...
function initMostrarDetalleSaldoHandler() {
    ...
    $(document).on('click', '.link-detalle-saldo', function () {
        ...
        var idConsultor = $(this).data('id-consultor');
        ...
        $.ajax({
            type: 'post',
            url: __AJAX_PATH__ + 'consultor/detalle_resumen_cuenta_corriente/',
            data: data,
            success: function (items) {
                ...
                $contenidoDetalle
                        .append(
                                $('<table id="table-contratos" dataexport-title="tramo-oc" \n\
                ...
                jQuery.each(items, function (index, item) {
                    ...
                    $contenidoDetalle.find('tbody')
                            .append($('<tr>')
                                    ...
                                    .append($('<td class="nowrap">').text(item['claseContrato']))
                                    .append($('<td class="nowrap">').text(item['numeroContrato']))
...
function initTable() {
    ...
    dt_init($('#table-contratos'), options);
}
