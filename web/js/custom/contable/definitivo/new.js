...
jQuery(document).ready(function () {
    ...
    $('form[name="adif_contablebundle_definitivo"]').validate();
...
function configSubmitButton() {
    ...
    $('#adif_contablebundle_definitivo_submit').on('click', function (e) {

        if ($('form[name=adif_contablebundle_definitivo]').valid()) {

            e.preventDefault();

            show_confirm({
                msg: '¿Desea guardar el definitivo?',
                callbackOK: function () {

                    if (validForm()) {
                        $('form[name=adif_contablebundle_definitivo]').submit();
                    }
                }
            });
...
function validForm() {
    ...
    // Si el Definitivo tiene renglones cargados
    if ($('.renglon-content').length === 0) {

        var options = $.extend({
            title: 'Ha ocurrido un error',
            msg: "Debe cargar al menos un renglón al definitivo."
        });
