...
$(document).ready(function () {
    ...
    $('[id^="adif_inventariobundle_hojaruta"][id$="_fechaVencimiento"]').datepicker('setStartDate', 'now');
});
...
function addItemForm() {
    ...
    for(var i in campos){
        ...
        if($select.val() !== ''){
            ...
            //var nombre = (typeof transformNombre === 'function') ? transformNombre(campos[i]):campos[i];
    ...
    $.post(url, data, function(resp) {
        if( resp.length > 0 ){ //Se obtiene un ARRAY de JSON:
            if(itemIncompleto  && ((typeof fixValidMR === 'function')?fixValidMR(data):true)){
                addItem(); //También se agrega el item incompleto
            }
            addMultiplesItems(resp);
        }else{
            if(msgError === 'No se encontraron registros que cumplan los filtros seleccionados'
               && itemIncompleto
               && campoDatoMaestro == 'materialRodante'){

                msgError = 'No se encontraron materiales rodantes para relevar en la Estación seleccionada. '+
...
function cargarCollectionItem(itemForm, $selector = false, datosItem = false ){
    ...
    for(var i in camposItemForm){
        ...
        if($selector){ //En caso de recibir el formulario del item:
            ...
            var valor = (typeof fixMrTransform === 'function')? fixMrTransform(id, datosItem):datosItem[id];
        }
...
function addDataTableItem($selector, index){
    ...
    for(var i in campos){
        ...
        var value = (typeof idToDenominacion === 'function')?
            idToDenominacion(id, $('#' + idSelectorValue).attr('value')):
            $('#' + idSelectorValue).attr('value');
...
function updateItemDeleteLinks(deleteLink) {
    ...
    deleteLink.each(function () {
        ...
        $(this).off("click").on('click', function (e) {
            ...
            //var deletableRow = $('div.prototype-item > div[index="'+index+'"]');
            var deletableRow = $('div.prototype-item').find('[id ^="' + id_prefix + tipoItems + index + '" ]').parent();
