...
class ExporterController extends BaseController {
    ...
    public function crearTabla($phpExcelObject, $options) {
        ...
        $titulo = 'Listado de Imputaciones Automaticas de Cobranzas (impreso el ' . $fecha->format('d/m/Y') . ' a las ' . $fecha->format('H:i:s') . ' hs.)';
        ...
        //Header extracto
        $phpExcelObject->getActiveSheet()->setCellValueByColumnAndRow($currentColumn, $currentRow, 'Movimientos registrados en el banco');
        ...
        //Header pendientes
        $phpExcelObject->getActiveSheet()->setCellValueByColumnAndRow($currentColumn, $currentRow, 'Comprobantes pendientes de pago');
        ...
        for ($index; $index < $tope; $index++) {
            ...
            $phpExcelObject->getActiveSheet()->getStyle('L' . $index)->applyFromArray($styleNumber);
        }


        $currentColumn = $this->__INIT_COLUMN__;
        $currentRow++;
        $totalHeaders = count($options['headers']) - 1;
        foreach ($options['headers'] as $numeroRow => $header) {
            $columnLetter = \PHPExcel_Cell::stringFromColumnIndex($currentColumn);
            if ($numeroRow > ($totalHeaders - 2)) {
                $phpExcelObject->getActiveSheet()->setCellValueByColumnAndRow($currentColumn, $currentRow - 1, $header->texto);
                $phpExcelObject->getActiveSheet()->mergeCells($columnLetter . ($currentRow - 1) . ':' . $columnLetter . $currentRow);
                $phpExcelObject->getActiveSheet()->getStyle($columnLetter . ($currentRow - 1) . ':' . $columnLetter . $currentRow)->applyFromArray($styleHeader);
            } else {
...
//                $phpExcelObject->getActiveSheet()->setCellValueExplicit(\PHPExcel_Cell::stringFromColumnIndex($this->__INIT_COLUMN_LETRA__ + 1 + $key) . $currentRow, $data[$key]);
            }
