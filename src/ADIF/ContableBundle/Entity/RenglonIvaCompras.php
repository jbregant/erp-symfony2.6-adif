...
class RenglonIvaCompras extends BaseAuditoria implements BaseAuditable {
    ...
    /**
     * @var double
     * @ORM\Column(name="gastos_bancarios", type="decimal", precision=10, scale=2, nullable=false)
     */
    ...
    /**
     * Devuelve si es valido el renglon IVA compras
     * @return boolean
     */    
    public function getMensajeInvalido() {
        ...
        if ($this->getIva21() <> 0) {
            if ($this->getGastosBancarios() <> 0) {
                if (!((($this->getIva21() < 0) && ($this->getGastosBancarios() < 0)) || (($this->getIva21() > 0) && ($this->getGastosBancarios() > 0)))) {
                    $result = [
                        'error' => true,
                        'mensaje' => 'El IVA 21 y los gastos bancarios tienen que estar ambos al debe o al haber'
                    ];                    
                }
            } else {
                $result = [
                    'error' => true,
                    'mensaje' => 'Si hay IVA 21 tiene que tener gastos bancarios'
                ]; 
            }
        }
        return $result;
